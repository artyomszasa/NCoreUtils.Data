...
public partial class FirestoreConverter
{
    ...
    private sealed class EntityFromValueImpl(TryGetValueDelegate tryGetValue, FirestoreConverter converter)
    {
        private TryGetValueDelegate TryGetValue { get; } = tryGetValue;

        private FirestoreConverter Converter { get; } = converter;
    ...
    internal object? EntityFromValue(TryGetValueDelegate tryGetValue, DataEntity entity)
    {
        ...
        return ctor.Instantiate(ctor.Properties
            .Select(p => entity.Properties.FirstByProperty(p.TargetProperty))
